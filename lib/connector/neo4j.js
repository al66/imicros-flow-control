...
class neo4j {
    ...
    async addVersion (version) {
        ...
        statement += "SET v.name = {name}, v.attributes = {attributes} ";
        statement += "RETURN v.uid AS id, v.name AS name ;";
        let result = await this.run(statement, params);
        return result;
    }
    ...
    async getVersions (process) {
        ...
        statement += "RETURN p.uid AS processId, v.uid AS versionId, v.name AS name ;";
        let result = await this.run(statement, params);
        return result;
    }
    ...
    async getProcesses (filter) {
        ...
        statement += "RETURN p.uid AS processId, p.name AS name, v.uid AS versionId, v.name AS versionName ;";
        let result = await this.run(statement, params);
    ...
    async updateInstance (instance) {
        ...
        statement += "ON CREATE SET i.createdAt = timestamp() ";
    ...
    async getInstances (selection) {
        ...
        statement += "RETURN i.processId AS processId, i.ownerId AS ownerId, i.uid AS instanceId, i.status AS status, i.createdAt AS createdAt ";
        let result = await this.run(statement, params);
        return result;
    }
