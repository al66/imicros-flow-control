...
class neo4j {
    ...
    constructor (broker, options) {
        ...
        if (!this.database) {
            this.database = {};
            this.database.uri = options?.uri ?? (process.env.NEO4J_URI || "bolt://localhost:7687");
            this.database.user = options?.user ?? (process.env.NEO4J_USER || "neo4j");
            this.database.password = options?.password ?? (process.env.NEO4J_PASSWORD || "neo4j");
        }
