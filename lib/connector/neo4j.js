...
class neo4j {
    ...
    async removeProcess (process) {
        ...
        let statement = "MATCH (n { ownerType: {ownerType}, ownerId: {ownerId} })-[:ASSIGNED]->(:Process { uid: {uid}, ownerId: {ownerId} }) ";
    ...
    async getEvent (event) {
        ...
        statement += "RETURN p.uid AS processId, e.name AS name, e.uid AS uid, e.position AS position, e.type AS type, e.direction AS direction, e.interaction AS interaction, e.ownerId AS ownerId, e.attributes AS attributes";
        let result = await this.run(statement, params);
    ...
    async addTask (task) {
        let type  = Constants.MANUAL_TASK;      // default
        type = (task.attributes && task.attributes.service && task.attributes.action) ? Constants.SERVICE_TASK : type;
        ...
        let statement = "MATCH (p:Process { uid: {processId}, ownerId: {ownerId} }) ";
        statement += "WITH p ";
        statement += "MERGE (t:Task { uid: {uid}, ownerId: {ownerId} })-[:ASSIGNED]->(p) ";
        statement += "SET t.name = {name}, t.attributes = {attributes}, t.type = {type} ";
    ...
    async getTask (task) {
        ...
        statement += "RETURN p.uid AS processId, t.name AS name, t.uid AS uid, t.type AS type, t.ownerId AS ownerId, t.attributes AS attributes";
        let result = await this.run(statement, params);
        this.logger.info("getTask", {statement: statement,params:params});
        if (result && result[0] && result[0].attributes && result[0].attributes !== ".") result[0].attributes = JSON.parse(result[0].attributes);
        if (result && result[0].attributes === ".") delete result[0].attributes;
    ...
    async addGateway (gateway) {
        ...
        let statement = "MATCH (p:Process { uid: {processId}, ownerId: {ownerId} }) ";
        statement += "WITH p ";
        statement += "MERGE (g:Gateway { uid: {uid}, ownerId: {ownerId} })-[:ASSIGNED]->(p) ";
        statement += "SET g.type = {type}, g.name = {name}, g.attributes = {attributes} ";
    ...
    async getElements (process) {
        ...
        let statement = "MATCH ()-[r:NEXT]->()-[:ASSIGNED]->(p:Process { uid: {processId}, ownerId: {ownerId} }) ";
        statement += "RETURN p.uid AS processId, p.ownerId AS ownerId, r.uid AS uid, r.type AS type ";
        statement += "UNION ALL ";
        statement += "MATCH (e)-[:ASSIGNED]->(p:Process { uid: {processId}, ownerId: {ownerId} }) ";
    ...
    async getSubscriptions (event) {
        ...
        let statement = "MATCH (e:Event { name: {name} })-[:ASSIGNED]->(p:Process { ownerId: {ownerId} }) ";
        statement += "RETURN p.uid AS processId, p.ownerId AS ownerId, e.uid AS elementId, e.type AS type, e.attributes AS attributes ";
        statement += "UNION ";
        statement += "MATCH (e:Event { name: {name} })-[:ASSIGNED]->(p:Process { core: true }) ";
        statement += "RETURN p.uid AS processId, p.ownerId AS ownerId, e.uid AS elementId, e.type AS type, e.attributes AS attributes ";
        let result = await this.run(statement, params);
    ...
    async updateInstance (instance) {
        ...
        statement += "SET i.status =  {status} ";
        statement += "RETURN i.processId AS processId, i.ownerId AS ownerId, i.uid AS instanceId, i.status AS status ";
        let result = await this.run(statement, params);
        return result;
    }
    ...
    async getInstances (selection) {
        ...
        if (selection.running && selection.failed) {
            statement += "WHERE i.status = '" + Constants.INSTANCE_RUNNING + "' OR i.status = '"  + Constants.INSTANCE_FAILED + "' ";
        } else if (selection.running) {
            statement += "WHERE i.status = '" + Constants.INSTANCE_RUNNING + "' ";
        } else if (selection.failed) {
            statement += "WHERE i.status = '" + Constants.INSTANCE_FAILED + "' ";
        }
        statement += "RETURN i.processId AS processId, i.ownerId AS ownerId, i.uid AS instanceId, i.status AS status ";
        let result = await this.run(statement, params);
        return result;
