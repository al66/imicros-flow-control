...
class neo4j {
    ...
    async getInstances (selection) {
        ...
        statement += "RETURN i.processId AS processId, i.versionId AS versionId, i.ownerId AS ownerId, i.uid AS instanceId, i.status AS status, i.createdAt AS createdAt ";
        let result = await this.run(statement, params);
        return result;
    }
